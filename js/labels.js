...
class LabelsManager {
    constructor() {
        ...
        this.selectAllLabelsBtn = document.getElementById('selectAllLabelsBtn'); // Nouveau bouton
    ...
    initEventListeners() {
        ...
        // Nouveau bouton pour sélectionner toutes les étiquettes
        if (this.selectAllLabelsBtn) {
            this.selectAllLabelsBtn.addEventListener('click', () => this.toggleSelectAll());
        }
    }
    ...
    /**
     * Sélectionne ou désélectionne tous les produits
     */
    toggleSelectAll() {
        // Si tous les produits sont déjà sélectionnés, désélectionner tout
        if (this.selectedProducts.length === this.products.length) {
            this.selectedProducts = [];
            this.selectAllLabelsBtn.textContent = "Sélectionner tout";
        } else {
            // Sinon, sélectionner tous les produits
            this.selectedProducts = this.products.map(product => product.id);
            this.selectAllLabelsBtn.textContent = "Désélectionner tout";
        }
    ...
    renderProductsList(searchTerm = '') {
        ...
        // Mise à jour du texte du bouton "Sélectionner tout"
        if (this.selectAllLabelsBtn) {
            // Vérifier si tous les produits filtrés sont sélectionnés
            const allFilteredSelected = filteredProducts.every(product => 
                this.selectedProducts.includes(product.id)
            );
            
            this.selectAllLabelsBtn.textContent = allFilteredSelected ? 
